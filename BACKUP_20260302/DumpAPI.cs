...
class Program
{
    static void Main()
    {
        try
        {
            ...
            var propFrameType = types.FirstOrDefault(t => t.Name == "cPropFrame");
            
            if (propFrameType != null)
            {
                var methods = propFrameType.GetMethods().Where(m => m.Name.Contains("GetRebarColumn")).ToList();
            ...
            {
                Console.WriteLine("cPropFrame type not found");
            }
        }
        catch (Exception ex)
        {
